    ...
    return (
        <div className='fixed top-0 left-0 z-[100] w-full h-screen bg-[#000000c2] flex justify-center '>
            <div className='flex items-center h-full lg:w-[50%] max-lg:px-3 py-10'>
                <div className='bg-[#121212] rounded-md lg:p-8 max-lg:p-4 flex flex-col gap-3 h-full overflow-auto scrollbar-none relative'>
                    ...
                    <p className='text-white'>Please copy the credentials below and paste it in the login form further after clicking the login button</p>
                    <div className='min-[1350px]:grid grid-cols-2 gap-3 w-full'>
                        <div className=''>
                            <label htmlFor="email">Email</label>
                            <div name='email' className='border border-white p-2.5 rounded-md flex gap-4 justify-between items-center '><span className='overflow-hidden max-[350px]:w-[150px]'>{email}</span> { copiedemail ? <p className='text-xs'>copied!</p>: <button onClick={() => handleCopy(email)} className='cursor-pointer'><FaRegCopy className=' hover:fill-white' /></button>}</div>
                        </div>
                        <div className=''>
